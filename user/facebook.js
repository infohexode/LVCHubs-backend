    ...
    try {
        ...
        if(!existUser){
            const newUser = new model({
              ...
              await newUser.save();
              return res.status(200).json({token: tokenForUser(newUser),name:newUser.name,id:newUser._id,url:newUser.imageUrl,email:newUser.email, loginFrom: "Facebook"});
        }
        
        return res.status(200).json({token: tokenForUser(existUser),name:existUser.name,id:existUser._id,url:existUser.imageUrl,email:existUser.email, loginFrom: "Facebook"});
    } catch (error) {
        next(error)
    }
}